...
async function auditTransition(mutation, action, userId, details = {}) {
  try {
    await db.execute(`
...
router.get('/mutations', async (req, res, next) => {
  try {
    ...
    res.json({ rows: await workflow.listMutations(req.query || {}) });
  } catch (error) {
    next(error);
  }
});
...
router.get('/mutations/:id', async (req, res, next) => {
  try {
    ...
    const mutation = await workflow.getMutation(req.params.id);
...
router.post('/mutations', async (req, res, next) => {
  try {
    ...
    const mutation = await workflow.createDraft(req.body || {}, req.user?.id);
    await auditTransition(mutation, 'mutation_brouillon_cree', req.user?.id);
...
router.put('/mutations/:id', async (req, res, next) => {
  try {
    if (!(await requirePermission(req, res, 'hr.mutation.create'))) return;
    const mutation = await workflow.updateDraft(req.params.id, req.body || {}, req.user?.id);
    await auditTransition(mutation, 'mutation_brouillon_modifie', req.user?.id);
    res.json(mutation);
  } catch (error) {
    if (sendWorkflowError(res, error)) return;
    next(error);
...
router.post('/mutations/:id/soumettre', async (req, res, next) => {
  try {
    ...
    const mutation = await workflow.submit(req.params.id, req.user?.id);
    await auditTransition(mutation, 'mutation_soumise', req.user?.id);
    res.json(mutation);
  } catch (error) {
    if (sendWorkflowError(res, error)) return;
    next(error);
...
async function approve(req, res, next) {
  try {
    ...
    const mutation = await workflow.approve(req.params.id, req.user?.id);
    const action = mutation.statut === workflow.STATUS.EFFECTIVE
      ...
      : mutation.statut === workflow.STATUS.NEEDS_CORRECTION
        ? 'mutation_a_corriger'
        : 'mutation_approuvee';
    await auditTransition(mutation, action, req.user?.id);
    res.json(mutation);
  } catch (error) {
    if (sendWorkflowError(res, error)) return;
    next(error);
...
async function refuse(req, res, next) {
  try {
    if (!(await requirePermission(req, res, 'hr.mutation.approve'))) return;
    const mutation = await workflow.refuse(req.params.id, req.user?.id, req.body?.motif || req.body?.motif_refus);
    await auditTransition(mutation, 'mutation_refusee', req.user?.id, { motif_refus: mutation.motif_refus });
    res.json(mutation);
  } catch (error) {
    if (sendWorkflowError(res, error)) return;
    next(error);
...
router.post('/mutations/:id/annuler', async (req, res, next) => {
  try {
    ...
    const mutation = await workflow.cancel(req.params.id, req.user?.id, req.body?.motif);
    await auditTransition(mutation, 'mutation_annulee', req.user?.id, { motif: mutation.motif_refus });
    res.json(mutation);
  } catch (error) {
    if (sendWorkflowError(res, error)) return;
    next(error);
...
router.post('/mutations/:id/appliquer', async (req, res, next) => {
  try {
    ...
    const mutation = await workflow.apply(req.params.id, req.user?.id);
...
router.post('/mutations/appliquer-echeances', async (req, res, next) => {
  try {
    if (!(await requirePermission(req, res, 'hr.mutation.apply'))) return;
    res.json(await workflow.applyDue(req.user?.id));
  } catch (error) {
    next(error);
  }
});
