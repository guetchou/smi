...
router.get('/mutations', async (req, res, next) => {
  try {
    const allowed = await Promise.all([
      hasPermission(req.user, 'hr.mutation.create'),
      hasPermission(req.user, 'hr.mutation.approve'),
      hasPermission(req.user, 'hr.mutation.apply'),
      hasPermission(req.user, 'hr.agent.update'),
    ]);
...
async function approve(req, res, next) {
  try {
    ...
    auditTransition(
      mutation,
      mutation.statut === workflow.STATUS.EFFECTIVE ? 'mutation_approuvee_et_appliquee' : 'mutation_approuvee',
      req.user?.id,
    );
    res.json(mutation);
  } catch (error) {
