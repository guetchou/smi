...
router.get('/periodes', async (_req, res, next) => {
  try { res.json(await db.query('SELECT * FROM periodes_paie ORDER BY annee DESC, mois DESC LIMIT 24')); }
  catch (error) { next(error); }
});
...
router.post('/periodes/:id/soumettre-dg', async (req, res, next) => {
  try {
    ...
    if (await canApprovePayrollPeriod(req.user)) {
      await db.execute(`UPDATE periodes_paie SET statut='validee_dg', soumis_dg_by=?, soumis_dg_at=NOW(), valide_dg_by=?, valide_dg_at=NOW(), updated_at=NOW() WHERE id=?`, [req.user.id, req.user.id, periode.id]);
    ...
    await db.execute(`UPDATE periodes_paie SET statut='soumis_dg', soumis_dg_by=?, soumis_dg_at=NOW(), updated_at=NOW() WHERE id=?`, [req.user.id, periode.id]);
    ...
    try {
      ...
      for (const user of dgs) await Promise.resolve(creerNotification({ type: 'NOTIF_PERIODE_PAIE', titre: `Masse salariale ${periode.mois}/${periode.annee} soumise`, message: `La masse salariale de ${periode.mois}/${periode.annee} est soumise pour validation (total net : ${new Intl.NumberFormat('fr-FR').format(synthese.total_net)} XAF).`, srcTable: 'periodes_paie', srcId: periode.id, destinataire_id: user.id }));
    } catch (_) {}
...
router.post('/periodes/:id/valider-dg', async (req, res, next) => {
  try {
    ...
    await updatePeriodeStats(periode.mois, periode.annee);
    await db.execute(`UPDATE periodes_paie SET statut='validee_dg', valide_dg_by=?, valide_dg_at=NOW(), notes=?, updated_at=NOW() WHERE id=?`, [req.user.id, notes || null, periode.id]);
...
router.post('/periodes/:id/cloturer', async (req, res, next) => {
  try {
    ...
    const nbNonPayes = Number((await db.queryOne("SELECT COUNT(*) AS c FROM bulletins_salaire WHERE mois=? AND annee=? AND statut != 'paye'", [periode.mois, periode.annee]))?.c || 0);
...
router.post('/periodes/:id/rouvrir-exception', async (req, res, next) => {
  try {
    ...
    await db.transaction(async tx => {
      ...
      await tx.execute('INSERT INTO audit_logs (table_name, record_id, action, details, user_id) VALUES (?,?,?,?,?)', ['periodes_paie', periode.id, 'ALERTE_SECURITE_REOUVERTURE', JSON.stringify({ motif: normalizedMotif, mois: periode.mois, annee: periode.annee }), req.user.id]);
    });
