...
router.get('/me/status', async (req, res) => {
  try {
    ...
    const assignment = await activeAssignment(employeId, workDate);
    res.json({ employe_id: employeId, work_date: workDate, local_date: parts.localDate, last_event: last, allowed_events: allowed, summary, assignment });
...
router.post('/events', async (req, res) => {
  try {
    ...
    const result = await engine.recordEvent({
      employeId, eventType, source: 'web', mode, timezone, utcOffsetMinutes: 60,
    ...
    res.status(result.idempotentReplay ? 200 : 201).json({ ...result.event, idempotent_replay: result.idempotentReplay, daily: calculated });
  } catch (error) { errorResponse(res, error); }
});
