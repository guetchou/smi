  ...
  try {
    ...
    const out = await db.transaction(async tx => {
      const r = await tx.execute(
        'INSERT INTO employes_enfants (employe_id,nom,prenom,date_naissance,sexe,est_charge,scolarise,observation) VALUES (?,?,?,?,?,?,?,?)',
        [id, nom, prenom, date_naissance, sexe, est_charge, scolarise, observation],
      );
      ...
      await tx.execute(
        'UPDATE employes SET nb_enfants=?, nb_enfants_charge=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
        [enfants.length, charge, id],
      );
...
router.post('/:id/documents', async (req, res, next) => {
  try {
    ...
    const out = await db.transaction(async tx => {
      const r = await tx.execute(
        'INSERT INTO employes_documents (employe_id,type_document,date_emission,date_expiration,statut,observation) VALUES (?,?,?,?,?,?)',
        [id, type_document, date_emission, date_expiration, statut, observation],
      );
...
router.post('/:id/diplomes', async (req, res, next) => {
  try {
    ...
    const payload = {
      etablissement: text(req.body?.etablissement),
      pays: text(req.body?.pays, 'Congo-Brazzaville') || 'Congo-Brazzaville',
      niveau: text(req.body?.niveau, 'autre') || 'autre',
      observation: text(req.body?.observation),
    };
    const id = Number(req.params.id);
    const out = await db.transaction(async tx => {
      const r = await tx.execute(
        'INSERT INTO employes_diplomes (employe_id,intitule,etablissement,pays,annee_obtention,niveau,observation) VALUES (?,?,?,?,?,?,?)',
        [id, intitule, payload.etablissement, payload.pays, annee, payload.niveau, payload.observation],
      );
      await touchAgent(id, tx);
...
router.post('/:id/experiences', async (req, res, next) => {
  try {
    ...
    const out = await db.transaction(async tx => {
      const r = await tx.execute(
        'INSERT INTO employes_experiences (employe_id,poste,entreprise,date_debut,date_fin,type_contrat,description) VALUES (?,?,?,?,?,?,?)',
        [id, poste, entreprise, date_debut, date_fin, type_contrat, description],
      );
      await touchAgent(id, tx);
...
router.post('/:id/avances/:aid/remboursements', async (req, res, next) => {
  try {
    ...
    const out = await db.transaction(async tx => {
      const r = await tx.execute(
        'INSERT INTO employes_avances_remboursements (avance_id,date,montant,notes,created_by) VALUES (?,?,?,?,?)',
        [avance.id, date, montant, notes, req.user?.id || null],
      );
      await tx.execute(
        'UPDATE employes_avances SET solde_restant=?, montant_rembourse=COALESCE(montant_rembourse,0)+?, statut=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
        [nouveauSolde, montant, nouveauStatut, avance.id],
      );
...
router.post('/:id/avances/:aid/decaisser', async (req, res, next) => {
  try {
    ...
    const position = req.body?.position_id
      ? await db.queryOne('SELECT id FROM positions WHERE id = ? AND actif = 1', [Number(req.body.position_id)])
      : await db.queryOne("SELECT id FROM positions WHERE actif=1 AND type IN ('caisse','banque') ORDER BY ordre LIMIT 1");
    ...
    if (bal != null) {
      ...
      const computed = await db.queryOne(`
    ...
    const opId = await db.transaction(async tx => {
      ...
      `, [libelle, tiers, montant, position.id, cat?.id || null, Number(req.params.id), req.user.id, req.user.id]);

      await tx.execute(
        "UPDATE employes_avances SET statut_workflow='decaisse', operation_id=?, updated_at=CURRENT_TIMESTAMP WHERE id=? AND operation_id IS NULL",
        [op.insertId, avance.id],
      );
      await tx.execute(`
        INSERT INTO cash_ledger (caisse_id, operation_id, type_mouvement, montant, solde_avant, solde_apres, reference, created_by)
        VALUES (?, ?, 'debit', ?, ?, ?, ?, ?)
      `, [position.id, op.insertId, montant, soldeBefore, soldeAfter, libelle, req.user.id]);
      const upd = await tx.execute(
        'UPDATE cashbox_balances SET solde_courant=?, derniere_operation_id=?, updated_at=CURRENT_TIMESTAMP WHERE caisse_id=?',
        [soldeAfter, op.insertId, position.id],
      );
      if (!upd.affectedRows) {
        await tx.execute(
          'INSERT INTO cashbox_balances (caisse_id, solde_courant, derniere_operation_id, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)',
          [position.id, soldeAfter, op.insertId],
        );
      }
      await audit(tx, 'employes_avances', avance.id, 'decaisser', {
        operation_id: op.insertId,
        montant,
...
router.post('/:id/conges', async (req, res, next) => {
  try {
    ...
    const overlap = await db.queryOne(
      "SELECT id, date_debut, date_fin FROM employes_conges WHERE employe_id=? AND statut IN ('demande','valide_sup','approuve') AND date_debut <= ? AND date_fin >= ? LIMIT 1",
      [Number(req.params.id), date_fin, date_debut],
    );
    ...
    if (type_conge === 'maladie') {
      const s = await congeSolde(req.params.id);
      if (s && nb_jours > s.maladie.solde && !force && !hasRole(req.user, 'admin')) return res.status(400).json({ error: `Solde maladie insuffisant : ${s.maladie.solde} j disponible(s), ${nb_jours} demandé(s)`, solde_maladie: s.maladie.solde, nb_jours });
      if (s && nb_jours > s.maladie.solde && force) type_conge = 'sans_solde';
    }

    const out = await db.transaction(async tx => {
      const r = await tx.execute(
        'INSERT INTO employes_conges (employe_id,type_conge,date_debut,date_fin,nb_jours,motif,notes,created_by) VALUES (?,?,?,?,?,?,?,?)',
        [Number(req.params.id), type_conge, date_debut, date_fin, nb_jours, motif, notes, req.user?.id || null],
      );
...
router.put('/:id/conges/:cid/valider-sup', async (req, res, next) => {
  try {
    ...
    await db.transaction(async tx => {
      await tx.execute(
        "UPDATE employes_conges SET statut='valide_sup', valide_sup_par=?, valide_sup_at=CURRENT_TIMESTAMP, valide_sup_notes=?, updated_by=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
        [req.user.id, notes || null, req.user.id, conge.id],
      );
...
router.put('/:id/conges/:cid/approuver', async (req, res, next) => {
  try {
    ...
    const overlap = await db.queryOne(
      "SELECT id, date_debut, date_fin FROM employes_conges WHERE employe_id=? AND id != ? AND statut='approuve' AND date_debut <= ? AND date_fin >= ? LIMIT 1",
      [Number(req.params.id), Number(req.params.cid), conge.date_fin, conge.date_debut],
    );
    ...
    const s = await db.transaction(async tx => {
      await tx.execute(
        "UPDATE employes_conges SET statut='approuve', approuve_par=?, approuve_at=CURRENT_TIMESTAMP, updated_by=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
        [req.user.id, req.user.id, conge.id],
      );
      const balance = await setLeaveCounters(req.params.id, tx);
      if (conge.type_conge === 'maladie') {
        ...
        await tx.execute(
          'UPDATE employes SET conges_maladie_pris=?, conges_maladie_solde=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
          [newPris, newSolde, Number(req.params.id)],
        );
      }
...
router.put('/:id/conges/:cid/refuser', async (req, res, next) => {
  try {
    ...
    await db.transaction(async tx => {
      await tx.execute(
        "UPDATE employes_conges SET statut='refuse', refuse_par=?, refuse_at=CURRENT_TIMESTAMP, refuse_motif=?, updated_by=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
        [req.user.id, motif, req.user.id, conge.id],
      );
...
router.put('/:id/conges/:cid/annuler', async (req, res, next) => {
  try {
    ...
    const s = await db.transaction(async tx => {
      await tx.execute(
        "UPDATE employes_conges SET statut='annule', annule_statut=?, annule_at=CURRENT_TIMESTAMP, annule_by=?, annule_motif=?, updated_by=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
        [conge.statut, req.user.id, motif, req.user.id, conge.id],
      );
      const balance = await setLeaveCounters(req.params.id, tx);
      if (conge.statut === 'approuve' && conge.type_conge === 'maladie') {
        ...
        await tx.execute(
          'UPDATE employes SET conges_maladie_pris=?, conges_maladie_solde=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
          [
            Math.max(0, num(emp?.conges_maladie_pris, 0) - num(conge.nb_jours, 0)),
            num(emp?.conges_maladie_solde, 15) + num(conge.nb_jours, 0),
    ...
    res.json({ ok: true, statut: 'annule', solde: s || await congeSolde(req.params.id) });
  } catch (e) { next(e); }
});
    ...
    const sanction = await db.transaction(async tx => {
      const r = await tx.execute(`
        INSERT INTO employes_sanctions
          (employe_id,type,date_sanction,motif_detaille,nb_jours_mise_a_pied,retenue_calculee,document_url,statut,created_by,created_at,updated_at)
        VALUES (?,?,?,?,?,?,?,'projet',?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)
...
router.put('/:id/sanctions/:sid', async (req, res, next) => {
  try {
    ...
    const updated = await db.transaction(async tx => {
      await tx.execute(
        'UPDATE employes_sanctions SET type=?, date_sanction=?, motif_detaille=?, nb_jours_mise_a_pied=?, retenue_calculee=?, document_url=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
        [type, date_sanction, motif_detaille, jours, retenue, document_url, sanction.id],
      );
...
async function transitionSanction(req, res, next, { roles, expected, nextStatus, action, requireMotif = false }) {
  try {
    ...
    await db.transaction(async tx => {
      if (requireMotif) {
        await tx.execute("UPDATE employes_sanctions SET statut=?, conteste_motif=?, updated_at=CURRENT_TIMESTAMP WHERE id=?", [nextStatus, motif, sanction.id]);
      } else {
        await tx.execute('UPDATE employes_sanctions SET statut=?, updated_at=CURRENT_TIMESTAMP WHERE id=?', [nextStatus, sanction.id]);
      }
...
router.put('/:id/sanctions/:sid/notifier', (req, res, next) => transitionSanction(req, res, next, {
  roles: ['admin', 'rh', 'dg'], expected: ['projet'], nextStatus: 'notifie', action: 'notifier',
}));
router.put('/:id/sanctions/:sid/contester', (req, res, next) => transitionSanction(req, res, next, {
  roles: ['admin', 'rh'], expected: ['notifie'], nextStatus: 'conteste', action: 'contester', requireMotif: true,
}));
router.put('/:id/sanctions/:sid/clore', (req, res, next) => transitionSanction(req, res, next, {
  roles: ['admin', 'dg'], expected: ['notifie', 'conteste'], nextStatus: 'clos', action: 'clore',
}));
    ...
    const created = await db.transaction(async tx => {
      const r = await tx.execute(`
        INSERT INTO employes_heures_sup
          (employe_id,mois,annee,date_heures,nb_heures,type,taux_majoration,montant_brut,statut,valide_par,motif,created_by,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)
      `, [agent.id, mois, annee, date_heures, nb_heures, type, rate, amount, autoValide ? 'valide' : 'saisi', autoValide ? req.user.id : null, motif || null, req.user.id]);
