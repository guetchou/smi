...
async function param(cle, def = null) {
  const r = await db.queryOne('SELECT valeur FROM parametres WHERE cle=?', [cle]);
...
router.get('/messages', async (req, res) => {
  try {
    ...
    const totalRow = await db.queryOne(`SELECT COUNT(*) as c FROM notif_messages ${where}`, params);
...
router.patch('/messages/:id/lire', async (req, res) => {
  try {
    const row = await db.queryOne(
    ...
    await db.execute(
      "UPDATE notif_messages SET statut='lue', acquitte_at=NOW(), acquitte_par=?, updated_at=NOW() WHERE id=? AND statut='non_lue'",
      [req.user.id, row.id]
    );
    res.json({ ok: true });
  } catch (e) {
...
router.patch('/messages/tout-lire', async (req, res) => {
  try {
    const res2 = await db.execute(
      "UPDATE notif_messages SET statut='lue', acquitte_at=NOW(), acquitte_par=?, updated_at=NOW() WHERE user_id=? AND statut='non_lue'",
      [req.user.id, req.user.id]
    );
    res.json({ ok: true, nb: res2.affectedRows });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.patch('/messages/:id/archiver', async (req, res) => {
  try {
    const row = await db.queryOne('SELECT id, user_id FROM notif_messages WHERE id=?', [req.params.id]);
    if (!row) return res.status(404).json({ error: 'Notification introuvable' });
    if (row.user_id !== req.user.id && !isAdmin(req.user))
      return res.status(403).json({ error: 'Accès refusé' });

    await db.execute(
      "UPDATE notif_messages SET statut='archivee', updated_at=NOW() WHERE id=?",
      [row.id]
    );
    res.json({ ok: true });
  } catch (e) {
...
router.delete('/messages/archivees', async (req, res) => {
  try {
    const res2 = await db.execute(
...
router.get('/alertes', async (req, res) => {
  try {
    ...
    const totalRow = await db.queryOne(`SELECT COUNT(*) as c FROM alertes_actives a ${where}`, params);
    const total = totalRow.c;
    const rows  = await db.query(
...
router.patch('/alertes/:id/acquitter', async (req, res) => {
  try {
    ...
    await db.execute(
      "UPDATE alertes_actives SET statut='acquittee', acquitte_par=?, acquitte_at=NOW(), updated_at=NOW() WHERE id=? AND statut='active'",
      [req.user.id, alerte.id]
    );

    await db.execute(
      "INSERT INTO audit_logs (table_name, record_id, action, details, user_id) VALUES ('alertes_actives',?,?,?,?)",
      [alerte.id, 'acquitted', JSON.stringify({ par: req.user.id }), req.user.id]
    );
...
router.patch('/alertes/:id/resoudre', async (req, res) => {
  try {
    if (!isAdmin(req.user)) return res.status(403).json({ error: 'Admin requis' });

    const alerte = await db.queryOne('SELECT id FROM alertes_actives WHERE id=?', [req.params.id]);
    if (!alerte) return res.status(404).json({ error: 'Alerte introuvable' });

    await db.execute(
      "UPDATE alertes_actives SET statut='resolue', resolu_at=NOW(), resolu_auto=0, updated_at=NOW() WHERE id=?",
      [alerte.id]
    );

    await db.execute(
      "INSERT INTO audit_logs (table_name, record_id, action, details, user_id) VALUES ('alertes_actives',?,?,?,?)",
      [alerte.id, 'resolved', JSON.stringify({ manual: true, par: req.user.id }), req.user.id]
    );
...
router.patch('/alertes/:id/override', async (req, res) => {
  try {
    if (!isAdmin(req.user)) return res.status(403).json({ error: 'Admin requis' });
    ...
    const alerte = await db.queryOne(
    ...
    await db.execute(
      "UPDATE alertes_actives SET override_par=?, override_at=NOW(), override_motif=?, updated_at=NOW() WHERE id=?",
      [req.user.id, motif.trim(), alerte.id]
    );

    await db.execute(
      "INSERT INTO audit_logs (table_name, record_id, action, details, user_id) VALUES ('alertes_actives',?,?,?,?)",
      [alerte.id, 'override', JSON.stringify({ par: req.user.id, motif: motif.trim() }), req.user.id]
    );
...
router.patch('/rappels/:id/acquitter', async (req, res) => {
  try {
    ...
    await db.execute(
      "UPDATE notif_rappels SET statut='acquitte', acquitte_par=?, acquitte_at=NOW(), updated_at=NOW() WHERE id=? AND statut IN ('declenche','retarde','escalade')",
      [req.user.id, rap.id]
    );

    await db.execute(
      "INSERT INTO audit_logs (table_name,record_id,action,details,user_id) VALUES ('notif_rappels',?,?,?,?)",
      [rap.id, 'acquitted', JSON.stringify({ par: req.user.id }), req.user.id]
    );
...
router.post('/rappels', async (req, res) => {
  try {
    if (!isAdmin(req.user)) return res.status(403).json({ error: 'Admin requis' });
    ...
    const id = await planifierRappel({ type, srcTable: src_table, srcId: src_id,
...
router.delete('/rappels/:id', async (req, res) => {
  try {
    if (!isAdmin(req.user)) return res.status(403).json({ error: 'Admin requis' });
    const rap = await db.queryOne('SELECT * FROM notif_rappels WHERE id=?', [req.params.id]);
    if (!rap) return res.status(404).json({ error: 'Rappel introuvable' });

    await db.execute(
      "UPDATE notif_rappels SET statut='annule', annule_motif=?, updated_at=NOW() WHERE id=?",
      [req.body?.motif ?? 'Annulation manuelle', rap.id]
    );
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
router.get('/regles', async (req, res) => {
  try {
    if (!isAdmin(req.user)) return res.status(403).json({ error: 'Admin requis' });
    const rows = await db.query('SELECT * FROM notif_regles ORDER BY famille, type', []);
    res.json(rows);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
router.patch('/regles/:type', async (req, res) => {
  try {
    if (!isAdmin(req.user)) return res.status(403).json({ error: 'Admin requis' });

    const regle = await db.queryOne('SELECT id FROM notif_regles WHERE type=?', [req.params.type]);
    ...
    await db.execute(
      `UPDATE notif_regles SET ${set}, updated_at=NOW() WHERE type=?`,
      [...fields.map(f => req.body[f]), req.params.type]
    );

    await db.execute(
      "INSERT INTO audit_logs (table_name,record_id,action,details,user_id) VALUES ('notif_regles',?,?,?,?)",
      [regle.id, 'update', JSON.stringify({ champs: fields, par: req.user.id }), req.user.id]
    );
...
router.put('/preferences', async (req, res) => {
  try {
    ...
    await setPreferences(req.user.id, data);
...
router.put('/canaux/:type/:canal', async (req, res) => {
  try {
    ...
    const regle = await db.queryOne('SELECT priorite_defaut FROM notif_regles WHERE type=?', [type]);
    ...
    `, [req.user.id, type, canal, actif ? 1 : 0]);
  ...
  try {
    await db.execute(`
      ...
      ON DUPLICATE KEY UPDATE
        user_id=VALUES(user_id), key_p256dh=VALUES(key_p256dh),
        key_auth=VALUES(key_auth), actif=1, erreur_410_at=NULL
    `, [req.user.id, endpoint, keys.p256dh, keys.auth,
        req.headers['user-agent'] ?? null]);

    await db.execute(
      "UPDATE user_preferences SET push_actif=1, updated_at=NOW() WHERE user_id=?",
      [req.user.id]
    );
...
router.delete('/push/unsubscribe', async (req, res) => {
  try {
    ...
    const nbRow = await db.queryOne(
      "SELECT COUNT(*) as c FROM push_souscriptions WHERE user_id=? AND actif=1",
      [req.user.id]
    );
    if (nbRow.c === 0) {
      await db.execute(
        "INSERT INTO user_preferences (user_id,push_actif,updated_at) VALUES (?,0,NOW()) ON DUPLICATE KEY UPDATE push_actif=0,updated_at=NOW()",
        [req.user.id]
      );
    }
...
router.get('/admin/summary', async (req, res) => {
  try {
    ...
    res.json({
      alertesActives,
      rappelsDus:     rappelsDusRow.nb,
      notifsNonLues:  notifsNonLuesRow.nb,
      envoisEnEchec:  envoisEnEchecRow.nb,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
router.get('/admin/envois', async (req, res) => {
  try {
    if (!isAdmin(req.user)) return res.status(403).json({ error: 'Admin requis' });
    ...
    const totalRow = await db.queryOne(`SELECT COUNT(*) as c FROM notif_envois ${where}`, params);
    const total = totalRow.c;
    const rows  = await db.query(
...
router.post('/admin/test', async (req, res) => {
  try {
    ...
    const ids = await creerNotification({
...
router.get('/admin/params', async (req, res) => {
  try {
    ...
    const rows = await db.query(`SELECT cle, valeur FROM parametres WHERE cle IN (${placeholders})`, cles);
    const obj = {};
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      const existingRows = await tx.query(
        `SELECT cle, valeur FROM parametres WHERE cle IN (${allowed.map(() => '?').join(',')})`,
        allowed
      );
      existingRows.forEach(r => { avant[r.cle] = r.valeur; });
    ...
    await db.execute(
      ...
      [JSON.stringify({ avant, apres }), req.user.id]
    );
