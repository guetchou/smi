...
function minutesInNightWindow(startUtc, endUtc, offsetMinutes, nightStart = '22:00', nightEnd = '05:00') {
  ...
  const ns = hhmmMinutes(nightStart);
  const ne = hhmmMinutes(nightEnd);
...
function workedSegments(events) {
  ...
  for (const event of ordered) {
    ...
    if ((event.event_type === 'break_start' || event.event_type === 'clock_out') && active) {
      segments.push([active, event]);
      active = null;
    }
  }
  return segments;
...
function nightMinutes(events, schedule) {
  ...
  return workedSegments(events).reduce((sum, [a, b]) => sum + minutesInNightWindow(
    a.occurred_at_utc,
    b.occurred_at_utc,
    a.utc_offset_minutes ?? 60,
    schedule.nuit_debut || '22:00',
    schedule.nuit_fin || '05:00'
  ), 0);
}
...
function scheduleMetrics(events, schedule, summary, calendar) {
  ...
  if (schedule && events.length && isWorkingDay) {
    ...
    const nominalDefault = start !== null && expectedEnd !== null
      ? Math.max(0, expectedEnd - start - Number(schedule.pause_minutes || 0))
      : 0;
    const nominal = calendar?.scheduled_minutes_override !== null && calendar?.scheduled_minutes_override !== undefined
      ? Number(calendar.scheduled_minutes_override)
      : nominalDefault;
  ...
  if (minDuration !== null && summary.lastOutUtc && summary.workedMinutes < minDuration && isWorkingDay) {
    anomalies.push({ type: 'insufficient_duration', minutes: summary.workedMinutes, min_minutes: minDuration });
  }
...
async function recalculateDay(employeId, workDate) {
  return db.transaction(async tx => {
    ...
    if (existing?.status === 'closed') {
      ...
      err.code = 'DAY_CLOSED'; err.status = 409; throw err;
    }
    ...
    for (const event of events) {
      ...
      if (event.mode && event.mode !== 'bureau' && !policy.modeAllowed(schedule, event.mode)) metrics.anomalies.push({ type: 'remote_not_authorized', event_id: event.id, mode: event.mode });
    }
    ...
    await tx.execute(
      ...
      [employeId, workDate, schedule?.schedule_id || schedule?.id || null, base.firstInUtc, base.lastOutUtc, base.workedMinutes,
       base.breakMinutes, metrics.lateMinutes, metrics.earlyLeaveMinutes, metrics.overtimeMinutes, night,
    ...
    await tx.execute(
      `DELETE FROM pointeuse_anomalies WHERE employe_id = ? AND work_date = ? AND status IN ('detected','to_justify')`,
      [employeId, workDate]
    );
    ...
    return {
      ...
      work_date: workDate,
      status,
      summary: { ...base, ...metrics, nightMinutes: night },
      schedule_id: schedule?.schedule_id || schedule?.id || null,
      calendar,
    };
  });
}
