...
function extractSummary(html, label) {
  ...
  const rowRe = new RegExp(`<tr[^>]*>[\\s\\S]*?<td[^>]*>\\s*${escaped}\\s*<\\/td>([\\s\\S]*?)<\\/tr>`, 'i');
...
function extractCompanyAddress(html) {
  const header = html.match(/<div[^>]*>\s*<h1>[\s\S]*?<\/h1>\s*<div[^>]*>([^<]+)<\/div>/i);
  return header ? decodeHtml(header[1]) : '';
}
...
function renderProfessionalPayrollHtml(legacyHtml) {
  ...
  const employeeName = extractInfo(legacyHtml, 'Employé');
  const poste = extractInfo(legacyHtml, 'Poste');
  const cnss = extractInfo(legacyHtml, 'N° CNSS');
  const contract = extractInfo(legacyHtml, 'Contrat');
  const paymentMode = extractInfo(legacyHtml, 'Mode paiement');
  const companyName = extractCompanyName(legacyHtml) || 'TOP CENTER';
  const address = extractCompanyAddress(legacyHtml);
