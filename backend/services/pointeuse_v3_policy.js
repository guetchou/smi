...
function dayList(value) {
  return new Set(String(value || '').split(',').map(v => Number(v.trim())).filter(v => v >= 1 && v <= 7));
}
...
async function calendarDay(executor, assignment, workDate) {
  if (assignment?.calendar_id) {
    const explicit = await executor.queryOne(
      `SELECT day_type, libelle, scheduled_minutes_override
       FROM pointeuse_calendar_days WHERE calendar_id = ? AND work_date = ?`,
  ...
  return {
    day_type: allowed.has(weekdayIso(workDate)) ? 'workday' : 'rest',
    libelle: null,
    scheduled_minutes_override: null,
    explicit: false,
  };
}
...
async function sitePolicy(executor, siteCode) {
  ...
  return executor.queryOne(
    `SELECT id, code, libelle, latitude, longitude, rayon_m, gps_requis
     FROM pointeuse_sites WHERE code = ? AND actif = 1 LIMIT 1`,
    [siteCode]
  );
}
...
async function evaluateLocation(executor, { siteCode, latitude, longitude, precisionGps }) {
  ...
  if (gpsRequired && (!Number.isFinite(lat) || !Number.isFinite(lon))) {
    throw attendanceError('Position GPS requise pour ce site', 'ATTENDANCE_GPS_REQUIRED', 400);
  }
  ...
  return {
    ...
    precision_gps: Number.isFinite(Number(precisionGps)) ? Number(precisionGps) : null,
  };
}
...
function modeAllowed(assignment, mode) {
  ...
  return assignment.mode_autorise === 'hybride' || assignment.mode_autorise === mode;
}
