(function () {
  ...
  function fillModal(row = null) {
    ...
    const selectedAgent = row?.employe_id || ''; document.getElementById('org-mutation-agent').innerHTML = '<option value="">— Sélectionner —</option>' + optionRows(state.agents, 'id', agent => `${agent.nom || ''} ${agent.prenom || ''} — ${agent.matricule || 'sans matricule'}`, selectedAgent); document.getElementById('org-mutation-agent').disabled = Boolean(row);
    document.getElementById('org-mutation-type').value = row?.type_mutation || 'modification'; document.getElementById('org-mutation-date').value = row?.date_effective || row?.date_effet || new Date().toISOString().slice(0, 10);
  ...
  function openModal(row = null) { ensureModal(); fillModal(row); document.getElementById('modal-org-mutation').classList.remove('hidden'); }
  ...
  function actionButtons(row) {
    const buttons = []; if (['brouillon', 'refuse', 'a_corriger'].includes(row.statut) && can('hr.mutation.create')) buttons.push(`<button class="btn btn-secondary btn-xs" data-action="edit" data-id="${row.id}">Modifier</button>`);
  ...
  async function saveDraft(event) {
    ...
    catch (error) { showError(error.message, true); } finally { button.disabled = false; }
  }
