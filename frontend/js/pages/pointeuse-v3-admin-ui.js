(function(){
  ...
  function table(entetes,lignes){return lignes.length?`<div class="p3a-scroll"><table class="p3a-table"><thead><tr>${entetes.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${lignes.join('')}</tbody></table></div>`:'';}
  ...
  function listeMotifs(){return table(['Code','Libellé','Catégorie','Payé','Validation requise','Actif','Actions'],
    (config?.event_reasons||[]).map(r=>`<tr><td>${esc(r.code)}</td><td>${esc(r.libelle)}</td><td>${esc(r.categorie)}</td><td>${oui(r.paye)}</td><td>${oui(r.validation_requise)}</td><td>${etat(r)}</td><td>${actions('reason',r)}</td></tr>`));}
  ...
  function html(){return `<div class="p3a-shell"><div class="p3a-head"><h2>Administration des temps</h2><span class="p3a-pill">Mode ${esc(config?.mode||'shadow')}</span></div><div class="p3a-grid">
    ...
    <section class="p3a-card"><h3>Affectation agent</h3>${listeAffectations()?`${listeAffectations()}<p class="p3a-sub">&nbsp;</p>`:''}<form id="p3a-assignment" class="p3a-form"><label>Agent<select name="employe_id" required><option value="">—</option>${(config?.employes||[]).map(e=>`<option value="${e.id}">${esc(e.matricule||e.id)} · ${esc(e.prenom||'')} ${esc(e.nom||'')}</option>`).join('')}</select></label><label>Planning<select name="schedule_id" required>${options(config?.schedules)}</select></label><label>Calendrier<select name="calendar_id"><option value="">—</option>${options(config?.calendars)}</select></label><label>Site<select name="site_code"><option value="">—</option>${(config?.sites||[]).map(x=>`<option value="${esc(x.code)}">${esc(x.code)} · ${esc(x.libelle)}</option>`).join('')}</select></label><label>Début<input name="date_debut" type="date" required></label><label>Fin<input name="date_fin" type="date"></label><label>Mode<select name="mode_autorise"><option>bureau</option><option>teletravail</option><option>terrain</option><option>hybride</option></select></label><button>Affecter</button></form></section>
    <section class="p3a-card p3a-wide"><h3>Motifs de pause et de sortie</h3><form id="p3a-reason" class="p3a-form" style="max-width:760px"><label>Code<input name="code" required placeholder="PAUSE-REPAS"></label><label>Libellé<input name="libelle" required></label><label>Catégorie<select name="categorie"><option value="pause">Pause</option><option value="sortie">Sortie</option></select></label><label>Payé<select name="paye"><option value="1">Oui</option><option value="0">Non</option></select></label><label>Validation requise<select name="validation_requise"><option value="0">Non</option><option value="1">Oui</option></select></label><button>Enregistrer</button></form>${listeMotifs()?`<p class="p3a-sub">&nbsp;</p>${listeMotifs()}`:''}</section>
  ...
  const CHAMPS={
    ...
    reason:r=>({code:r.code,libelle:r.libelle,categorie:r.categorie,paye:Number(r.paye)?'1':'0',validation_requise:Number(r.validation_requise)?'1':'0'}),
  };
  const COLLECTIONS={site:'sites',schedule:'schedules',calendar:'calendars',reason:'event-reasons'};
  const SOURCES={site:'sites',schedule:'schedules',calendar:'calendars',reason:'event_reasons'};
  const FORMULAIRES={site:'p3a-site',schedule:'p3a-schedule',calendar:'p3a-calendar',reason:'p3a-reason'};

  function remplir(kind,id){
    const r=(config?.[SOURCES[kind]]||[]).find(x=>String(x.id)===String(id));
  ...
  function bind(){
    ...
    const forms={site:'/admin/sites',schedule:'/admin/schedules',calendar:'/admin/calendars',assignment:'/admin/assignments',period:'/admin/periods',reason:'/admin/event-reasons'};
