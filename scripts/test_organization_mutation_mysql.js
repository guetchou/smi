  ...
  try {
    ...
    const organization = {
      ...
      assertManagerActive(id) {
        ...
        return { id: managerId, nom: 'Manager', prenom: 'CI', poste: 'Chef cible', departement: 'CI Target', site: 'Site B' };
      },
      assertNoCycle() {},
    };
    ...
    try { await workflow.approve(mutationId, rhUserId); } catch (error) { selfApproval = error; }
    ...
    await db.execute('UPDATE employes_mutations SET date_effet=?, date_effective=? WHERE id=?', [today, today, mutationId]);
