...
assert(ui.includes('initializing: false'));
assert(ui.includes('if (state.initialized || state.initializing) return;'));
assert(ui.includes("console.warn('[org-mutation-workflow-ui] initialization retry'"));
assert(ui.includes('while (!state.initialized && attempts < 120)'));
assert(!ui.includes("console.error('[org-mutation-workflow-ui]'"));
const referencesLoad = ui.indexOf('await loadReferences();');
const initializedCommit = ui.indexOf('state.initialized = true;');
